      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-bold tracking-tight text-slate-900">Berita</h1>
            <p className="mt-2 text-sm text-slate-500">Kelola berita dan pantau status publikasi.</p>
          </div>
          <Button asChild variant="outline" className="bg-white hover:bg-slate-50">
            <Link href="/workspace/berita/tambah" prefetch={false}>
        ...
        <Suspense fallback={<AdminNewsTableSkeleton />}>
          <AdminNewsTable currentPage={currentPage} userId={userId} />
        </Suspense>
      </section>
    </div>
  )
...
async function AdminNewsTable({
  ...
  return (
    <>
      <p className="mb-4 text-sm text-slate-500">{totalCount} berita tersimpan.</p>
            ...
            {paginatedItems.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="px-6 py-12 text-center">
                  <h3 className="text-lg font-semibold text-slate-900">Belum ada berita</h3>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Button variant="outline" size="icon-sm" asChild>
                        <Link href={`/workspace/berita/${item.id}/edit`} prefetch={false}>
      ...
      <div className="mt-6">
        <AdminPagination
          ...
          getHref={(page) => `/workspace/berita?page=${page}`}
        />
      </div>
    </>
  )
...
function AdminNewsTableSkeleton() {
  return (
    <>
      <Skeleton className="mb-4 h-4 w-40 rounded-full bg-slate-200" />
      <div className="overflow-hidden rounded-xl border border-slate-200">
        <div className="space-y-3 p-4">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="grid grid-cols-[5rem_minmax(0,1.6fr)_1fr_1fr_5rem_6rem_5rem] gap-3">
